# Classes for computing training statistics
#
# References
# ----------
# https://github.com/trzy/FasterRCNN//pytorch/FasterRCNN/statistics.py

from collections import defaultdict
import numpy as np

from .model.math_utils import intersection_over_union
...
class TrainingStatistics:
  ...
  def on_training_step(self, loss):
    ...
    self._rpn_regression_losses.append(loss.rpn_bbox)
    self._detector_class_losses.append(loss.detector_class)
    self._detector_regression_losses.append(loss.detector_bbox)
...
class PrecisionRecallCurveCalculator:
  ...
  def __init__(self):
    ...
    self._unsorted_predictions_by_class_id = defaultdict(list)

    # True number of objects by class for all images in dataset
    self._object_count_by_class_id = defaultdict(int)

  def _compute_correctness_of_predictions(self, scored_bboxes_by_class_id, gt_boxes):
    unsorted_predictions_by_class_id = {}
    object_count_by_class_id = defaultdict(int)
    ...
    for gt_box in gt_boxes:
      object_count_by_class_id[gt_box.class_id] += 1

    for class_id, scored_boxes in scored_bboxes_by_class_id.items():
      # Get the ground truth boxes corresponding to this class
      gt_boxes_this_class = [ gt_box for gt_box in gt_boxes if gt_box.class_id == class_id ]
      ...
      unsorted_predictions_by_class_id[class_id] = [ (scored_boxes[i][4], is_true_positive[i]) for i in range(len(scored_boxes)) ]
        
    return unsorted_predictions_by_class_id, object_count_by_class_id

  def add_image_results(self, scored_bboxes_by_class_id, gt_boxes):
    """
    ...
    Parameters
    ----------
    scored_bboxes_by_class_id : dict
    ...
    unsorted_predictions_by_class_id, object_count_by_class_id = self._compute_correctness_of_predictions(
      scored_bboxes_by_class_id = scored_bboxes_by_class_id,
      gt_boxes = gt_boxes) 
    for class_id, predictions in unsorted_predictions_by_class_id.items():
      self._unsorted_predictions_by_class_id[class_id] += predictions
    for class_id, count in object_count_by_class_id.items():
      self._object_count_by_class_id[class_id] += object_count_by_class_id[class_id]

  def _compute_average_precision(self, class_id):
    # Sort predictions in descending order of score
    sorted_predictions = sorted(self._unsorted_predictions_by_class_id[class_id], key = lambda prediction: prediction[0], reverse = True)
    num_ground_truth_positives = self._object_count_by_class_id[class_id]
  ...
  def compute_mean_average_precision(self):
    ...
    """
    average_precisions = []
    for class_id in self._object_count_by_class_id:
      average_precision, _, _ = self._compute_average_precision(class_id = class_id)
      average_precisions.append(average_precision)
    return np.mean(average_precisions)
  
  def plot_precision_vs_recall(self, class_id, class_name = None, interpolated = False):
    """
    ...
    """
    average_precision, recall_array, precision_array = self._compute_average_precision(class_id = class_id, interpolated = interpolated)

    # Plot raw precision vs. recall
    import matplotlib.pyplot as plt
    label = "{0} AP={1:1.2f}".format("Class {}".format(class_id) if class_name is None else class_name, average_precision)
  ...
  def plot_average_precisions(self, class_id_to_name): 
    # Compute average precisions for each class
    labels = [ class_id_to_name[class_id] for class_id in self._object_count_by_class_id ]
    average_precisions = []
    for class_id in self._object_count_by_class_id:
      average_precision, _, _ = self._compute_average_precision(class_id = class_id)
      average_precisions.append(average_precision)
  ...
  def print_average_precisions(self, class_id_to_name):
    # Compute average precisions for each class
    labels = [ class_id_to_name[class_id] for class_id in self._object_count_by_class_id ]
    average_precisions = []
    for class_id in self._object_count_by_class_id:
