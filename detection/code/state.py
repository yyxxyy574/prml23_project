...
# References
# ----------
# https://github.com/trzy/FasterRCNN//pytorch/FasterRCNN/state.py
...
def _load_keras_biases(hdf5_file, layer_name):
  ...
  for keypath, node in hdf5_file[primary_keypath].items():
    if keypath.startswith("conv") or keypath.startswith("dense"):
      ...
      return torch.from_numpy(biases).cuda()
  return None
...
def _load_vgg16_from_bart_keras_model(filepath):
  ...
  for layer_name in keras_layers:
    ...
    if weights is not None and biases is not None:
      state["_feature_extractor._" + layer_name + ".weight"] = weights 
      state["_feature_extractor._" + layer_name + ".bias"] = biases
    else:
  ...
  if weights is not None and biases is not None:
    ...
    state["_detector._fc1.weight"] = weights
    state["_detector._fc1.bias"] = biases
  else:
  ...
  if weights is not None and biases is not None:
    ...
    state["_detector._fc2.weight"] = weights.permute([ 1, 0 ])
    state["_detector._fc2.bias"] = biases
  else:
...
def _load_vgg16_from_caffe_model(filepath):
  state = {}
  caffe = torch.load(filepath)

  # Attempt to load all layers
  mapping = {
    "features.0.":    "_feature_extractor._block1_conv1",
    "features.2.":    "_feature_extractor._block1_conv2",
    "features.5.":    "_feature_extractor._block2_conv1",
    "features.7.":    "_feature_extractor._block2_conv2",
    "features.10.":   "_feature_extractor._block3_conv1",
    "features.12.":   "_feature_extractor._block3_conv2",
    "features.14.":   "_feature_extractor._block3_conv3",
    "features.17.":   "_feature_extractor._block4_conv1",
    "features.19.":   "_feature_extractor._block4_conv2",
    "features.21.":   "_feature_extractor._block4_conv3",
    "features.24.":   "_feature_extractor._block5_conv1",
    "features.26.":   "_feature_extractor._block5_conv2",
    "features.28.":   "_feature_extractor._block5_conv3",
    "classifier.0.":  "_detector._fc1",
    "classifier.3.":  "_detector._fc2"
  }
...
class BestWeightsTracker:
  ...
  def save_best_weights(self, model):
    if self._best_state is not None:
      torch.save(self._best_state, self._filepath)
